    ...
    <div class="box-body">
      <div class="row">
        ...
        <div class="form-group col-xs-12 col-sm-4">
          <select id="idItem" name="idItem" class="form-control" placeholder="Item">
            <?php
              $produtos->getProdutosWithIdItem();
            ?>
          </select>
        </div>
	...
	if ($pkCount == 0) {
		echo '<tr>
              ...
              foreach ($_SESSION['itens'] as $produto => $quantidade) {
                ...
                <td>'. $quantidade .'</td>
                <td>
                <input type="hidden" id="idItem" name="idItem['.$produto.']" value="'.$produto.'" /> 
                <input type="hidden" id="qtd" name="qtd['.$produto.']" value="'.$quantidade.'" />
                <a title="Remover item código '. $produto .'." href="../../App/Database/remover.php?remover=carrinho&id='.$produto.'"><i class="fa fa-trash text-danger"></i></a>
                </td>
                </tr>';
                $cont = $cont + 1;
              }
