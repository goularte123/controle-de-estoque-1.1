  ...
  */
  ...
  public function listProdutos(){
   ...
   public function getProdutosWithIdItem(){
    $query = "SELECT produtos.*, itens.idItens AS idItem FROM produtos INNER JOIN itens ON itens.Produto_CodRefProduto = produtos.CodRefProduto WHERE produtos.Ativo = 1 AND produtos.PublicProduto = 1 GROUP BY produtos.CodRefProduto";
    $result = mysqli_query($this->SQL, $query) or die ( mysqli_error($this->SQL));
 
    if($result){
      while ($row = mysqli_fetch_array($result)) {
        echo '<option value="'.$row['idItem'].'" '.$selected.' >'.$row['NomeProduto'].'</option>';
      }
    }
  }

  public function getProdutoByIdItem($id){
    $query = "SELECT produtos.* FROM itens INNER JOIN produtos ON produtos.CodRefProduto = itens.Produto_CodRefProduto WHERE produtos.Ativo = 1 AND produtos.PublicProduto = 1 AND itens.idItens = $id LIMIT 1";
    $result = mysqli_query($this->SQL, $query) or die ( mysqli_error($this->SQL));
  ...
  public function DelProdutos($value)
    ...
    public function getProduto($CodRefProduto)
    {
      $query = "SELECT * FROM produtos WHERE CodRefProduto = $CodRefProduto AND Ativo = 1 AND PublicProduto = 1 LIMIT 1";
      $result = mysqli_query($this->SQL, $query) or die(mysqli_error($this->SQL));
      return mysqli_fetch_array($result);
    }
